...
if not os.path.exists('Verisae/VerisaeQuote'):
    os.makedirs('Verisae/VerisaeQuote')
...
<WorkOrderActions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <work_orders>
    ...
    if response.status_code == 200:
        print('Accept successfully.')
        with open('Verisae/VerisaeQuote/work_order_accept.xml', 'w') as file:
            file.write(response.text)
        with open('Verisae/VerisaeQuote/work_order_accept.xml', 'w') as file:
            file.write(xml_request)
    else:
