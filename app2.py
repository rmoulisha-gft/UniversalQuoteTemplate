        ...
        with col1:
            ...
            with incol1:
                if st.button("Approve", key="3"):
                    ...
                    st.experimental_rerun()
            with incol2:
                if st.button("Decline", key="4"):
                    declinetime = datetime.now()
                    decline = declinetime.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                    updateParent(st.session_state.ticketN, st.session_state.editable, st.session_state.NTE_Quote, "1900-01-01 00:00:00.000",  "1900-01-01 00:00:00.000", decline, st.session_state.ticketDf["BranchName"].get(0))
                    st.session_state.ticketN = ""
                    st.experimental_rerun()
            incol1, incol2, incol3 = st.columns([1,1,1])
        category_table_data = []
        ...
        if(incol1.button("Open PDF")):
            with col1:
                ...
                if general_description is not None:
                    paragraph = Paragraph("Incurred Workdescription: "+str(st.session_state.workDesDf["Incurred"].get(0))+"<br/>Proposed Workdescription: "+str(st.session_state.workDesDf["Proposed"].get(0)), paragraph_style)
                else:
